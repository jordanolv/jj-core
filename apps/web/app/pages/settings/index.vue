...
// Create settings items dynamically from features
const featureSettingsItems: SettingsItem[] = featuresWithSettings.value.map(feature => ({
  name: feature.name,
  description: `Paramètres ${feature.name}`,
  icon: feature.icon,
  route: `/settings/${feature.id}`,
...
const settingsItems: SettingsItem[] = [
  {
    name: 'Navigation',
    description: 'Personnaliser le menu du bas',
    icon: '🧭',
...
<template>
  <div class="px-4 pb-4 sm:px-6 sm:pb-6">
    <div class="mx-auto max-w-2xl">
      ...
      <!-- Settings List -->
      <div class="space-y-1">
        <template v-for="item in settingsItems" :key="item.name">
          <NuxtLink
            v-if="item.route !== '#' && !item.action"
            :to="item.route"
            class="block w-full"
          >
            <div class="group flex items-center gap-3 px-4 py-3 rounded-xl transition-all active:bg-white/5 sm:hover:bg-white/5"
          >
            <!-- Icon avec gradient -->
            <div
